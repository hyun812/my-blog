...
화면 크기에 따라 그리드 차원을 동적으로 계산합니다.

```typescript
useEffect(() => {
  const calculateGridDimensions = () => {
    const { usableWidth, usableHeight } = calculateUsableSpace(isEditMode);
    setGridDimensions(createGridDimensions(usableWidth, usableHeight));
  };
